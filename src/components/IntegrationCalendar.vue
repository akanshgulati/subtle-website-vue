<template>
  <section class='flex flex-flow-column integration-section flex-center'>
    ...
    <template v-if="!code">
      <div class="pv-30 center" >
        <img src="../assets/images/loading.gif" alt="" width="100px">
        <p>
          You will be redirected to <strong>Google Calendar</strong> page
          to authorize <strong>Subtletab</strong> to access your
          calendar data.
        </p>
        <h4 v-show="counter">Redirect in {{counter}}...</h4>
      </div>
    </template>
    <template v-else>
    ...
    <div class="pv-20">
      <div class="flex flex-justify-center">
        <input type="text" readOnly id="auth-code" onclick="this.select()" :value="code" class="mar-0">
        <button class="btn btn-group auth-copy" @click="copyCode">{{copyButtonTxt}}</button>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getAuthUrl() {
        const url = 'https://api.subtletab.com/integrations/calendar'
        const self = this;
        Http(url).then(resp => {
          self.counter = 4
          self.startCounter();
